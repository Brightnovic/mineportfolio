...
def verify_recaptcha(response):
    ...
    result = requests.post(recaptcha_url, data=payload)
...
def homePage(request):
    ...
    if request.method == 'POST':
        recaptcha_response = request.POST.get('recaptcha', None)
        if recaptcha_response and verify_recaptcha(recaptcha_response):
            ...
            if form.is_valid():
                ...
                if email_send(data):
                    ...
                    return JsonResponse({'success': False, 'errors': "Failed to send email. Please try again."})
            else:
                return JsonResponse({'success': False, 'errors': form.errors})
        return JsonResponse({'success': False, 'errors': "Invalid reCAPTCHA. Please try again."})
